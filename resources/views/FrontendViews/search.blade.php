...
@section("content")
    ...
    <section id="page_content">
        ...
        <div class="px-20 mt-5 wrapper">
            <h2 class="text-3xl mb-3 text-gray-500">{{$data->total()}} Ads found for <b>{{$request->q}}</b></h2>
            @if(!$data->isEmpty())
                @foreach($data as $key=>$row)
                    <div class="flex justify-between relative p-2 border-solid border-2 border-green-500 mb-4 single_item">
                        <div class="w-1/3 featured_image">
                            <a href="{{route('details.get', $row->slugurl)}}"><img src="{{$row->get_images[0]->img_name}}" class="w-full"></a>
                        </div>
